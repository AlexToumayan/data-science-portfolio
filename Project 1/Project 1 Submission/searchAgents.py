    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
             ...
             nextVisitedCorners = list(visited_corners)
            
             if nextPosition in self.corners:
                    if nextPosition not in visited_corners:
                       nextVisitedCorners.append(nextPosition)
            
             successor_state = (nextPosition, tuple(nextVisitedCorners))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    total_heuristic_dis = 0 
    position = current_position

    while unvisited_corners:
        distances = [(manhattan_distance(position, corner), corner) for corner in unvisited_corners]
        min_distance, closest_corner = min(distances)
        total_heuristic_dis += min_distance
        position = closest_corner
        unvisited_corners.remove(closest_corner)
    
    return total_heuristic_dis
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodList:
        ...
        for other_food in foodList:
            max_distance = max(max_distance, manhattanDistance(food, other_food))

    heuristic_value = closest_distance + max_distance
